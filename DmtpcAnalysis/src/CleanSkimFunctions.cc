...
TH2 * dmtpc::analysis::cleanskim::Functions::cleanImage(const TH2 * raw_img, const TH2 * bias, int & nkilled, double raw_rms,  double raw_mean, const TH2 * raw_overscan, double raw_os_mean, double os_bias_mean, const Config * conf)
{
  ...
  TFile * fpixel = new TFile("/scratch2/gmuraru/dmtpc_software/DmtpcSkim/m3_neutron_R1063010.m3ped.root", "read");
  TH2F * per_pix_ped = (TH2F*)fpixel->Get("cam0_pedRms");
...
bool dmtpc::analysis::cleanskim::Functions::checkSpark(int ev, const char * cam_id, double mean, double last_mean,  double os_spark_mean, const  Config * conf, dmtpc::Quality::RunQuality &runQ)
{
  
  bool spark = ((mean-last_mean) / last_mean >   conf->getDoubleMap("spark_cut",cam_id, 1.01) || runQ.isSpark(ev));
...
/*
double dmtpc::analysis::cleanskim::Functions::getMaxHVCurrent(dmtpc::analysis::MySQLRead * mysql, const char * table, TTimeStamp * start, TTimeStamp * end){

  double value = mysql->getValue(table,start,end);
...
//  int c = 0; 
  ...
  for (int i = 1; i < nargs; i++)
  {
    ...
    if (args[i][0]=='-')
    {
      ...
      if (strcmp(args[i],"-dd")==0)
	{
	  setup->dbdir = args[++i];
	  printf("Using dbdir: %s\n",setup->dbdir);
	}
      if (strcmp(args[i],"-dn")==0)
	{
	  setup->dbname = args[++i];
