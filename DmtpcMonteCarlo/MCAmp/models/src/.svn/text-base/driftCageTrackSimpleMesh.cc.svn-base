...
int main(int nargs, char ** args) 
{
  ...
  Model m("driftcagetrksm","driftcagetrksm"); 
  ...
  ChargeSensitiveAmplifier cr112(13,50e3,50e6); //13 mV/pC, 50 us falloff, 50 MHz bandwidth
  FastAmplifier r2e(80,50e6); //gain of 80 50 MHz bandwidth
  ...
  m.add("cathode", new Box(new TVector3(0,0,2.125),new TVector3(7.5,7.5,0.25)), &cathode, &steel); 
  m.add("anode", new Box(new TVector3(0,0,-0.125),new TVector3(7.5,7.5,0.25)), &anode, &steel); 
  ...
  m.add("mesh", new SimpleMesh(new TVector3(0,0,gap), pitch, radius,NULL,int(7/pitch),int(7/pitch),rotation), &ground, &steel,false,true); 

  //the last boolean argument makes this a bounding box
  m.add("bbox", new Box(new TVector3(0,0,1),new TVector3(8,8,3)), 0, &cf4,true); 
  ...
  int solved_ok = (redo ? solver.solve(&m) : solver.loadSolution(&m));  
  
  if (solved_ok)
  {
    printf("Solver returned %d, aborting\n",solved_ok); 
    return 1; 
  }
