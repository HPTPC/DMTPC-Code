...
struct dmtpc::mc::retrim::TrackMaker::particle 
{
           particle * parent; 
...
static double getT(double T, double dist, double mass_in_amu)
{
  ...
  double v =  sqrt(2*T/m) * 300;  //c = 300 mm/ns
  return dist / v; 
}


static void printTree(FILE * f, dmtpc::mc::retrim::TrackMaker::particle * p, int offset)
{
  ...
  for(std::list<dmtpc::mc::retrim::TrackMaker::particle*>::iterator it = p->daughters.begin(); it!=p->daughters.end(); it++)
  {
...
static double estimateElectronicLoss(const dmtpc::mc::retrim::TableReader * srim, double Estart, double distance, double ratio, double * nuke = 0) 
{
  ...
  for (int i = 0; i < nsegs; i++)
  {
    ...
    double frac = E/ (this_Nloss + this_Eloss); 
    if (frac > 1) frac = 1; 
...
static int whichSRIM(int atom, int nsrim, const dmtpc::mc::retrim::TableReader **srim) 
{
...
int dmtpc::mc::retrim::TrackMaker::fillTree(particle * p, bool record)
{
...
int dmtpc::mc::retrim::TrackMaker::enableHistograms(double binsize_in_mm, int nbins)
{
...
void dmtpc::mc::retrim::TrackMaker::clearHistograms() 
{
...
dmtpc::mc::retrim::TrackMaker::TrackMaker(const CollisionReader * trim_coll, int nsrim, const TableReader ** srim_tables, double pressure_ratio ) 
{
  setTRIM(trim_coll); 
...
dmtpc::mc::retrim::TrackMaker::~TrackMaker()
{
  delete stats; 
...
double dmtpc::mc::retrim::TrackMaker::getIonization(double x0, double x1, double y0, double y1, double z0, double z1) const
{
  double total = 0; 
...
int dmtpc::mc::retrim::TrackMaker::clearTrack() 
{
...
int dmtpc::mc::retrim::TrackMaker::makeTrack(int itrack, const TVector3 * origin, const TVector3 * direction, double t0) 
{
...
void dmtpc::mc::retrim::TrackMaker::fillIonizHists()
{
...
void dmtpc::mc::retrim::TrackMaker::transform(const TVector3 * origin, const TVector3 * dir)
{
...
std::vector<TObject*> dmtpc::mc::retrim::TrackMaker::draw(const char * opt) const
{
...
void dmtpc::mc::retrim::TrackMaker::fillElectrons(TH1 * fillme) const 
{
...
int dmtpc::mc::retrim::TrackMaker::getElectron(int i, double & x, double & y, double & z, double & t, double & E, double & px, double & py, double & pz)
{
...
double dmtpc::mc::retrim::TrackMaker::E() const
{
  return trim->getE(); 

}
int dmtpc::mc::retrim::TrackMaker::ntracks() const
{
...
int dmtpc::mc::retrim::TrackMaker::makeElectrons(const IonizationModel * m)
{
  electron_x.clear(); 
  electron_p.clear(); 
