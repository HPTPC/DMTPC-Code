void fitTRIMhists(const char * out, const char * dir, int max=553) 
{
  ...
  dmtpc::mc::retrim::HistogramReader::readIONIZ(TString::Format("%s/IONIZ0.txt",dir), &ion_ioniz, &recoil_ioniz); 
  ...
  for (int i = 0; i < max; i++) 
  {
    ...
    if (i > 0)
    {
      dmtpc::mc::retrim::HistogramReader::readIONIZ(TString::Format("%s/IONIZ%d.txt",dir,i), &ion_ioniz, &recoil_ioniz); 
      total_ioniz->Reset(); 
