...
{ 
  ...
  gStyle->SetOptStat(0); 
  dmtpc::mc::retrim::CollisionReader c(TString::Format("data/f/30torr/many/%dkeV/COLLISON%d.txt",energy,track)); 
  dmtpc::mc::retrim::TableReader * tables[2]; 

  tables[0] = new dmtpc::mc::retrim::TableReader("data/srim/f_in_cf4_30torr.txt"); 
  tables[1] = new dmtpc::mc::retrim::TableReader("data/srim/c_in_cf4_30torr.txt"); 

  
  dmtpc::mc::retrim::TrackMaker t(&c,2, tables,0.97); 
  t.setVerbose(1); 
...
//  t.getStats()->Draw("ElecLoss:predictedElecLoss","predictedElecLoss < 0.1","colz"); 
  ...
  dmtpc::mc::retrim::HistogramReader::readIONIZ(TString::Format("data/f/30torr/many/%dkeV/IONIZ%d.txt",energy,track),&true_ion, &true_recoil); 
  ...
  dmtpc::mc::retrim::HistogramReader::readPHONON(TString::Format("data/f/30torr/many/%dkeV/PHONON%d.txt",energy,track),&phonon_true_ion, &phonon_true_recoil); 
