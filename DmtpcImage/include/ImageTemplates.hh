#ifndef DMTPC_IMAGE_TEMPLATE_HH
#define DMTPC_IMAGE_TEMPLATE_HH
  ...
  namespace templated
  {
   ...
   vector<TH2*> * valueFit(InputIterator first, InputIterator last, int total = -1, TF1 * fn = NULL, const char * fitpars = "NQ", int maxalloc = 1<<16, bool verbose = false) 
   {
     ...
     for (int i = 0; i < npar; i++)
     {
       ...
       (*answer)[i] = dmtpc::image::tools::newTH2StealSize(hist,'D',name,name); 
     }

     (*answer)[npar] = dmtpc::image::tools::newTH2StealSize(hist,'D',"chisq","chisq"); 
...
//            cout << j << " " <<  idx << endl; 
      ...
      for (int j = 0; j < max; j++)
      {
        ...
        TH1 * fithist = dmtpc::image::tools::newTH1StealType(hist,"fithist","fithist",int(maxes[j]-mins[j] + 1), mins[j],maxes[j]); 
   ...
   TH2 * histStackNthElement(InputIterator first, InputIterator last, int nth, int total = -1, int maxalloc = 1024, bool verbose = false) 
   {
    ...
    TH2 * answer = dmtpc::image::tools::newTH2StealType(hist, "hnth","hnth", hist->GetNbinsX(), hist->GetXaxis()->GetXmin(), hist->GetXaxis()->GetXmax(), 
