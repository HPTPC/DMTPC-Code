...
//dir is the path to the directory with the raw data files in it
  ...
  //pixflag enables and disables the creation of the branch pixval. if pixflag is true, pixval will be stor    ed. If not, pixval will be ignored to save disk space.
  Bool_t pixflag = 0;

  //Ask the user if pixval will be stored. if yes, pixval will be stored
  char choice = '';
  std::cout<<endl;
  std::cout<<"Do you wish to store the intensity value of every pixel?"<<endl;
  std::cout<<"The size of the output file without this value is more than ***1000*** times smaller than the output file with such value"<<endl;
  std::cout<<"Do you wish to store the intensity value? y/n";
  std::cin>>choice;

  //If choose yes, set pixflag to true(1). If choose no, setpixflag to false(0). If choose others, prompt     the user to make choice again until 5 times limit is reached
  if (choice=='y')
  {
    pixflag = 1;
  }
  ...
  {
    short asdf =0;
  for (asdf = 0; (asdf<5)&&(choice!='y')&&(choice!='n');asdf++)
  {
    std::cout<<"Invalid input! Please choose YES(y) or NO(n). ";
    std::cin>>(char)choice;
  ...
  for (int fi=0;fi<runs;fi++){
    ...
    continue; //go onto next run if it doesn't
     ...
     //Stupid C++ does not recognise that the value of a variable is not constant or static if the value of it is passed twice. If it is passed to a const variable only once, C++ still knows that the value comes from a non-static variable, hence it will throw out an exception and terminates the program. Stupid C++. 
  ...
  //create branch pixval only if user chose to
    if (pixflag==1)
    {
