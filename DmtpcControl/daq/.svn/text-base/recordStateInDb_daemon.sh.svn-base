...
taskName[0]=SimpleDAQ
taskName[1]=CalcScopeBias
taskName[2]=RunCtrl
taskName[3]=fakeDAQ
...
function evalOneTask {
    ...
    pid=-1
    cpuSec=-1
    for line in $(ps -ef | grep -v grep  | grep -v  vi | grep -v  emacs| grep $taskNameX | awk '{print $1","$2","$8","$7}'); do
	nProc=$[ $nProc +1]  
	#echo $nProc $line
...
function scanMachine {
    ...
    xx=`df -BM /data |tail -n 1 |awk '{print $4}'`
    freeDisc=${xx:0:$((${#xx}-1))}   # skip units 'G'
    ...
    evalOneTask 2 ${taskName[2]}
    evalOneTask 3 ${taskName[3]}
...
function makeDbEntry() {
    ...
    $MYSQL  -BNe "INSERT INTO ${tableName} ( 
       ...
       ${numTask[2]}, ${pidTask[2]}, ${secTask[2]},
       ${numTask[3]}, ${pidTask[3]}, ${secTask[3]},
       '${remark}') ;"
}
