...
while read RUN
do
SCRIPT=$WORKDIR/qsubout2/scripts/jobscript_${TAG}_${RUN}.sh
...
printf "${WORKDIR}bin/${PROG} ${SKIMDIR}${FILE} ${SAVEDIR}${RUN}.root > ${WORKDIR}qsubout2/log/${RUN}.out 2> ${WORKDIR}qsubout2/err/${RUN}.err \n" >> ${SCRIPT} 
