{
  ...
  //cam 2
  TFile f("/scratch2/gmuraru/dmtpc_software/Analyses/field_uniformity/output/testband/2flush2x2.root");
  ...
  if(cut){
    ...
    t->Draw("mean:Entry$>>meanhist(200,0,200,100,-5,5)","mean<0 && mean>-5.0","colz");
  }
  else{
    t->Draw("mean:Entry$>>meanhist(200,0,200,100,-5,5)","","colz");
  }
  c1.cd(2);
  if(cut){
    int nbins = 3463;//t->GetEntries("mean < 100 && mean > -100");
    t->Draw("rms:Entry$>>rmshist(200,0,200,100,11,14)","mean <100 && mean > -100","colz");
  }
  else{
    t->Draw("rms:Entry$>>rmshist(200,0,200,100,11,14)","","colz");
  }
