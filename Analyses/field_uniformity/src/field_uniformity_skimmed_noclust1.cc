...
int main(int argc, char** argv){
  ...
  double sat_threshold = 5000;

  //for spark cutting via playlist

  for(int i = 0; i < ncam; i++){
    summed_data[i] = new TH2F(Form("summed_image_%d",i),Form("Summed Source Data Cam %d",i),1542,0, 3084,1542,0,3084);
    //manual histograms with no spark clusters or images
    manual_summed_data[i] = new TH2F(Form("manual_summed_image_%d",i),Form("Summed Source Data Cam %d",i),1542,0, 3084,1542,0,3084);
    add_counter[i] = new TH2D(Form("add_counter_%d",i), Form("Count of Bins Summed_%d",i), 1542,0, 3084, 1542, 0, 3084);

    summed_avg_data[i] = new TH2F(Form("summed__avg_image_%d",i),Form("Summed Averaged Source Data Cam %d",i),1542,0, 3084,1542,0,3084);
    summed_avg_data[i]->SetBit(TH1::kIsAverage);
    
    //manual histograms with no tracks at all
    noclust_sum[i] = new TH2D(Form("noclust_sum_%d",i),Form("Summed Data Excluding Clusters Cam %d",i),1542,0, 3084, 1542, 0, 3084);
    noclust_add_counter[i] = new TH2D(Form("noclust_add_counter_%d",i), Form("Count of Bins Summed_%d (No Clusters)",i), 1542,0, 3084, 1542, 0, 3084);
  ...
  TString rawname = skimfile;
  rawname.ReplaceAll("/scratch1/darkmatter/dmtpc/production/skimout/skim/m3/skim/2016/","/scratch3/darkmatter/dmtpc/data/m3/2016/01/raw/");
  rawname.ReplaceAll("skim.root",".root");

  dmtpc::analysis::SkimDataset * d = new dmtpc::analysis::SkimDataset;
  dmtpc::core::Dataset * rd = new dmtpc::core::Dataset;
    
 
    d->openRootFile(skimfile);
	...
	if(d->event()->ntracks(c) > 0){
	  ...
	  for(int t = 0; t < d->event()->ntracks(c); t++){
		...
		if(d->event()->image(c)->GetBinContent(d->event()->cluster(c)->getCluster(t)->pixels.at(bin)) > sat_threshold){
