void biasOS_pedestal(){
  ...
  d->open("/scratch1/darkmatter/dmtpc/data/m3/2016/m3_gain_R1038009.raw.root");
  
  TFile * biasOS_analysis = new TFile("biasOS_analysis.root","RECREATE");
  cout << d->nbias() << endl;
  for(int i = 0; i < d->nbias(); i++){
    cout << "i=" << i <<  endl;
    d->getbiasOS();
    // dmtpc::core::Image* myimage = d->biasRaw(camDAQ);                                                                                                                                               
    TH2I* myimage = d->biasOS(camDAQ)->getHist();
    myimage->Print("b");
    
    TH1F* intensity=new TH1F("intensity","intensity",1000,0,5000);
