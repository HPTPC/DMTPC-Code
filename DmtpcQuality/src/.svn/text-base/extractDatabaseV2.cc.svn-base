...
int main(int argc, char ** argv){
  ...
  //argv[4] = database name (optional). defaults to first and last run num in passed list
    ...
    //----------set up ------
  int readout = 2;//update to array for different binnings
  TString db_dir;
  char * year;
  char * month;
  if(argc < 4){
    std::cout<<"Usage: bin/extractDatabaseV2 year month"<<std::endl;
    return 0;
  }
  else{
    year = argv[2];
  ...
  for(unsigned int run = 0; run < run_nums.size(); run++){
    ...
    int exp4;//4 shooter exposure value
    if(have_sql_exp){
      exp4 = (int)run_db.getValue("run_start","expoTime4sh",runCondition);
      exp4 /= 1000;//convert to seconds
    }
    else{
      exp4 = exp;
    }
    ...
    for(int ch = 0; ch < nchan; ch++){
      ...
      for(int ev = nbias; ev < im_count.size(); ev++){
	if(ev%20==0) std::cout<<"Event: "<<ev<<std::endl;
	//get event time range
	uint32_t e_start = e_tstamps.at(ev).first;
	uint32_t e_end = e_start+exp+readout;
	store_cmd = "INSERT INTO EVENTS (run_id, ev, ev_start_s, ev_start_ns) ";
	store_cmd += Form("VALUES (%d, %d, %d, %d); ", run_nums.at(run).Atoi(), ev-nbias, e_tstamps.at(ev).first, 0);
	sqlite.storeValues(store_cmd);
	...
	if(is_spark){
	  ...
	  std::cout<<Form("VALUES (%d, %d, %d, %d, %f, %d)", run_nums.at(run).Atoi(), ev-nbias, ch, is_spark, last_spark_value, last_spark_time)<<std::endl;
	  ...
	  spark_out<<Form("VALUES (%d, %d, %d, %d, %f, %d)", run_nums.at(run).Atoi(), ev-nbias, ch, is_spark, last_spark_value, last_spark_time)<<std::endl;
	}
	
	store_cmd = "INSERT INTO SPARK_EVENTS (run_id, ev, channel, spark, last_spark_value, last_spark_time)";
	store_cmd += Form("VALUES (%d, %d, %d, %d, %f, %d)", run_nums.at(run).Atoi(), ev-nbias, ch, is_spark, last_spark_value, last_spark_time);
	sqlite.storeValues(store_cmd);
      }
